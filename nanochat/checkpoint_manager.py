...
def find_largest_model(checkpoint_dir):
    ...
    model_dirs = [p for p in checkpoint_dir.iterdir() if p.is_dir()]
    if not model_dirs:
        raise FileNotFoundError(f"No checkpoints found in {checkpoint_dir}")
    # 1) normally all model tags are of the form d<number>, try that first:
    candidates = []
    for model_dir in model_dirs:
        match = re.match(r"d(\d+)", model_dir.name)
        if match:
            model_depth = int(match.group(1))
            candidates.append((model_depth, model_dir.name))
    if candidates:
        candidates.sort(key=lambda x: x[0], reverse=True)
        return candidates[0][1]
    # 2) if that failed, take the most recently updated model:
    model_dirs.sort(key=lambda path: path.stat().st_mtime, reverse=True)
    return model_dirs[0].name
...
def find_last_step(checkpoint_dir):
    ...
    if not checkpoint_files:
        raise FileNotFoundError(f"No checkpoints found in {checkpoint_dir}")
    last_step = max(int(f.stem.split("_")[-1]) for f in checkpoint_files)
    return last_step
